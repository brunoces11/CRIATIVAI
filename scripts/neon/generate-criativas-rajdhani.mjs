...
const url = "https://raw.githubusercontent.com/google/fonts/main/ofl/rajdhani/Rajdhani-Regular.ttf";
const file = "scripts/neon/Rajdhani.ttf";
const out = "src/data/criativas-rajdhani-outline.json";
...
await fs.mkdir("src/data", { recursive: true }); await fs.writeFile(out, JSON.stringify({ text: "CRIATIVAS", font: "Rajdhani", weight: 400, paths: chunks.flatMap((p) => p.split(/(?=M)/g).filter(Boolean)) }, null, 2));
